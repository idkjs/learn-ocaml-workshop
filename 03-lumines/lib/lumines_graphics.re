...
let only_one: ref(bool) = (ref(false): ref(bool));
...
let dimensions = (game: Game.t) => {
  ...
  row: (game.height + extra_part_size_above) * part_size.row,
...
let init_exn = game => {
  ...
  if (only_one^) {
    failwith("Can only call init_exn once");
  } else {
    only_one := true;
  };
  let {Point.col, row} = dimensions(game);
  Graphics.open_graph(Printf.sprintf(" %dx%d", row, col));
};

let filled_square_to_rgb = (filled_square: Filled_square.t) =>
  Graphics.(
    switch (filled_square.color, filled_square.sweeper_state) {
...
let draw_part = (filled_square, ~from) => {
  ...
  switch (filled_square.Filled_square.sweeper_state) {
  | To_sweep
  | Swept =>
...
let draw_sweeper = (game: Game.t) => {
  let height = game.height * pixels_per_square;
  let pos = Sweeper.cur_pos(game.sweeper);
  Point.For_drawing.fill_rect(
...
let draw = game => {
  ...
  set_color(black);
  let dims = dimensions(game);
  Point.For_drawing.fill_rect(black, Point.For_drawing.origin, dims);
  List.iter(List.range(0, game.Game.width), ~f=col =>
    List.iter(List.range(0, game.Game.height), ~f=row =>
  ...
  List.iter(List.range(0, game.Game.width), ~f=col =>
    List.iter(List.range(0, game.Game.height), ~f=row =>
      switch (Board.get(game.Game.board, {row, col})) {
      | None => ()
      | Some(color) =>
        draw_part(
          color,
