...
module Expect_test_helpers = {
  let parse_and_print = (input, ~parser_, ~sexp_of_t) =>
    (
      switch (Angstrom.parse_string(parser_, input)) {
      | Ok(result) => sexp_of_t(result)
      | Error(s) =>
        %message
        "Failed to parse"(input: string, ~error=s: string)
      }
    )
    |> print_s;
};
...
let%expect_test "prefix" = {
  ...
  test(":foo@bar.com ");
  %expect
  {| (User (nickname foo) (user (((host (Hostname bar.com)) (user ()))))) |};
  test(":foo ");
  %expect
  {| (User (nickname foo) (user ())) |};
  test(":foo.bar.com ");
  %expect
  {| (Server foo.bar.com) |};
  test(":camlbot!caml@somehost ");
  %expect
  {| (User (nickname camlbot) (user (((host (Hostname somehost)) (user (caml)))))) |};
...
let%expect_test "message" = {
  ...
  test("NICK camlbot");
  %expect
  {| ((prefix ()) (command NICK) (params (camlbot))) |};
  test("USER camlbot * * :My Cool Bot");
  %expect
  {| ((prefix ()) (command USER) (params (camlbot * * "My Cool Bot"))) |};
  test("JOIN :##somechannel");
  %expect
  {| ((prefix ()) (command JOIN) (params (##somechannel))) |};
