...
let init_exn = () => {
  ...
  let height = play_area_height / block_size;
  let width = play_area_width / block_size;
  Game.create(~height, ~width, ~initial_snake_length=3, ~amount_to_grow=3);
};

let draw_block = ({Position.row, col}, ~color) => {
  open Constants;
  let col = col * block_size;
  let row = row * block_size;
...
let draw_header = (~game_state) => {
  open Constants;
  let header_color =
    switch ((game_state: Game_state.t)) {
  ...
  let header_text = Game_state.to_string(game_state);
...
let draw_apple = apple => {
  let apple_location = Apple.location(apple);
...
let render = game => {
  ...
  let apple = Game.apple(game);
  let game_state = Game.game_state(game);
  let snake_locations = Snake.locations(snake);
