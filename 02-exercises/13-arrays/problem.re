...
let () = {
  let array = Array.create(~len=5, "hello");
...
let%test "Testing double..." = {
  let array = [|1, 1, 1|];
  double(array);
  [%compare.equal: array(int)]([|2, 2, 2|], array);
};

let%test "Testing double..." = {
  let array = [|1, 2, 3, 4, 5|];
  double(array);
  [%compare.equal: array(int)]([|2, 4, 6, 8, 10|], array);
};
...
let%test "Testing double_selectively..." = {
  ...
  [%compare.equal: array(int)]([|1, 2, 1|], array);
};

let%test "Testing double_selectively..." = {
  let array = [|1, 2, 3, 4, 5|];
  double_selectively(array, [0, 2, 4]);
  [%compare.equal: array(int)]([|2, 2, 6, 4, 10|], array);
};
...
let () = {
  let matrix = Array.make_matrix(~dimx=5, ~dimy=3, "hello");
...
let%test "Testing double_matrix..." = {
  let matrix = [|[|1, 2, 3|], [|1, 1, 1|]|];
  double_matrix(matrix);
  [%compare.equal: array(array(int))](
    [|[|2, 4, 6|], [|2, 2, 2|]|],
    matrix,
  );
};
