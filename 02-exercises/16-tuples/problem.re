...
let example: int_and_string_and_char = (
  (5, "hello", 'A'): int_and_string_and_char
);
...
let int_pair: pair(int) = ((5, 7): pair(int));
let string_pair: pair(string) = (("foo", "bar"): pair(string));
let nested_char_pair: pair(pair(char)) = (
  (('a', 'b'), ('c', 'd')): pair(pair(char))
);
