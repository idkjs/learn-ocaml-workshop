...
let%test "Testing option_concatenate..." =
  switch (option_concatenate(Some("hello"), Some("world"))) {
  | Some("helloworld") => true
  | _ => false
  };

let%test "Testing option_concatenate..." =
  switch (option_concatenate(None, Some("world"))) {
  | None => true
  | _ => false
  };

let%test "Testing option_concatenate..." =
  switch (option_concatenate(Some("hello"), None)) {
  | None => true
  | _ => false
  };
...
let () = {
  ...
  let string2 = "world";
  assert(
    [%compare.equal: string]("helloworld", concatenate(string1, string2)),
  );
  assert(
...
let () =
  assert(
    String.(==)(
      "hi",
      better_labeled_concatenate(~string1="h", ~string2="i", ()),
    ),
  );
