...
let handle_keys = (game: Game.t, ~game_over) =>
  every(~stop=game_over, 0.01, ~f=() =>
    switch (Snake_graphics.read_key()) {
    | None => ()
    | Some(key) =>
      let set_direction = dir => Game.set_direction(game, dir);
      switch (key) {
      | 'w' => set_direction(Up)
...
let run = () => {
  let game = Snake_graphics.init_exn();
  Snake_graphics.render(game);
