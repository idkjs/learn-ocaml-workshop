...
let render =
  ...
  let m = config.Config.grid_size_in_px;
...
let init_event_handlers =
    (config: Config.t, screen: Screen.t, wad, game_impl: Game_impl.t(_)) => {
...
let initialize_and_main_loop = (config: Config.t, wad, game: Game_impl.t(_)) => {
  let board_div =
...
let main = game => {
  ...
  let wad = Wad.create(Config.default);
  Html.window##.onload :=
    Html.handler(_ =>
      let () = initialize_and_main_loop(Config.default, wad, game);
