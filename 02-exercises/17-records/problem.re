...
let an_example: person = (
  {first_name: "Cotton-eyed", last_name: "Joe", age: 22, number_of_cars: 0}: person
);

/* We can use the "." operator to get the value of a field out of a record:
   VARIABLE.FIELD */
let age: int = (an_example.age: int);
let () = assert(age == 22);
...
module For_testing = {
  let test_ex1: person = (
    {first_name: "Jan", last_name: "Saffer", age: 55, number_of_cars: 0}: person
  );

  let test_ex1': person = ({...test_ex1, age: 30}: person);

  let test_ex2: person = (
    {first_name: "Hugo", last_name: "Heuzard", age: 4, number_of_cars: 55}: person
  );

  let test_ex2': person = ({...test_ex2, number_of_cars: 61}: person);
